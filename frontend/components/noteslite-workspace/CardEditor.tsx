...
import EmojiIcon, { getEmojiAssetName } from '@/frontend/components/emoji-icon';
...
export default function CardEditor({ colName, cardTitle, cardData, setCardData, closeEditor, triggerToast, handleDeleteCard }: CardEditorProps) {
  ...
  const [slashMenu, setSlashMenu] = useState<{ x: number; y: number; index?: number; mode?: 'insert' | 'replace' } | null>(null);
  ...
  const replaceBlock = (type: string, index: number) => {
    const newBlocks = [...data.blocks];
    newBlocks[index] = { t: type, v: '', done: false };
    setCardData({ ...data, blocks: newBlocks });
    setFocusIndex(index);
    setSlashMenu(null);
    triggerToast(`Block added: ${type}`);
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    ...
    if (e.key === '/' && target.tagName === 'TEXTAREA' && target.value.trim() === '') {
      e.preventDefault();
      const rect = target.getBoundingClientRect();
      setSlashMenu({ x: rect.left, y: rect.bottom + 4, index, mode: 'replace' });
    }
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          ...
          <button className="noteslite-tbtn" onClick={() => {
            if (editorBodyRef.current) {
              ...
              setSlashMenu({ x: rect.left + 80, y: window.innerHeight / 2, mode: 'insert' });
            }
          }}>＋ Block</button>
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="noteslite-block-add-btn" onClick={() => {
                    if (editorBodyRef.current) {
                      const rect = editorBodyRef.current.getBoundingClientRect();
                      setSlashMenu({ x: rect.left + 80, y: window.innerHeight / 2, index: index + 1, mode: 'insert' });
                    }
          ...
          <div className="noteslite-slash-menu" style={{ display: 'flex', flexDirection: 'column', maxHeight: '320px', left: Math.min(slashMenu.x, window.innerWidth - 260), top: Math.min(slashMenu.y, window.innerHeight - 360) }}>
            ...
            <div className="noteslite-slash-menu-items" style={{ overflowY: 'auto', flex: 1 }}>
              {[
                ...
                <div key={item.id} className="noteslite-slash-item" onClick={() => {
                  if (slashMenu?.mode === 'replace' && slashMenu.index !== undefined) {
                    replaceBlock(item.id, slashMenu.index);
                  } else {
                    insertBlock(item.id, slashMenu?.index);
                  }
                }}>
